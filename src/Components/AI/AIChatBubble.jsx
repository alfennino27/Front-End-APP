    ...
    try {
      if (prop.type === 'potong_piutang_spk') {
        ...
        const res = await fetch(`${baseUrl}/ai/chat/todo/confirm`, {
          method: 'POST', headers: authHeaders({ 'Content-Type': 'application/json' }),
          body: JSON.stringify({
            items: (prop.items || []).map((it) => ({ project_id: it.project_id, category: it.category, text: it.text })),
            created_by_uid: getUid(),
          }),
        });
        updateProposal(msgId, idx, { _status: res.ok ? 'saved' : 'error' });
                      ...
                      return (
                        <div key={i} style={{ fontSize: 13, padding: '8px 10px', marginBottom: 6, borderRadius: 8,
                          ...
                          {isPengeluaran ? (
                            ...
                            <>
                              <div style={{ fontWeight: 700, marginBottom: 4 }}>✅ Tambah To Do ({(p.items || []).length} item)</div>
                              {(p.items || []).map((it, k) => (
                                <div key={k} style={{ marginBottom: 4 }}>
                                  <b>{it.nama_barang || it.project_id}</b>{it.kode_invoice ? <span style={{ opacity: 0.7 }}> ({it.kode_invoice})</span> : null}
                                  <div style={{ fontSize: 12 }}><span style={{ opacity: 0.7 }}>[{it.category}]</span> {it.text}</div>
                                </div>
                              ))}
                              {p.skipped && p.skipped.length > 0 && (
                                <div style={{ marginTop: 4, fontSize: 11.5, color: '#dc3545' }}>{p.skipped.length} item dilewati (sudah ada / bukan Ongoing).</div>
                              )}
                            </>
                          ) : isTarget ? (
                            <>
